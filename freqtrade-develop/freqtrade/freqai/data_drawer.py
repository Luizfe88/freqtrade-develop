...
class FreqaiDataDrawer:
    ...
    def load_global_metadata_from_disk(self):
        ...
        if exists:
            try:
                ...
                logger.warning("Global metadata file is corrupted. Starting from scratch.")
        return {}
    ...
    def load_drawer_from_disk(self):
        ...
        if exists:
            try:
                ...
                logger.warning("Pair dictionary file is corrupted. Starting from scratch.")
        else:
        ...
        if self.freqai_info.get("write_metrics_to_disk", False):
            ...
            if exists:
                try:
                    ...
                    logger.warning("Metric tracker file is corrupted. Starting from scratch.")
            else:
        ...
        if exists:
            try:
                ...
                try:
                    ...
                    logger.warning("Backup historical predictions file was also corrupted. "
    ...
    def save_historic_predictions_to_disk(self):
        ...
        """
        temp_path = self.historic_predictions_path.with_suffix(".tmp")
        with temp_path.open("wb") as fp:
            cloudpickle.dump(self.historic_predictions, fp, protocol=cloudpickle.DEFAULT_PROTOCOL)
        temp_path.replace(self.historic_predictions_path)
    ...
    def save_metric_tracker_to_disk(self):
        ...
        """
        temp_path = self.metric_tracker_path.with_suffix(".tmp")
        with self.save_lock:
            ...
            temp_path.replace(self.metric_tracker_path)

    def save_drawer_to_disk(self) -> None:
        """
        Save data drawer full of all pair model metadata in present model folder.
        """
        temp_path = self.pair_dictionary_path.with_suffix(".tmp")
        with self.save_lock:
            with temp_path.open("w") as fp:
                rapidjson.dump(
                    self.pair_dict, fp, default=self.np_encoder, number_mode=rapidjson.NM_NATIVE
                )
            temp_path.replace(self.pair_dictionary_path)

    def save_global_metadata_to_disk(self, metadata: dict[str, Any]):
        """
        Save global metadata json to disk
        """
        temp_path = self.global_metadata_path.with_suffix(".tmp")
        with self.save_lock:
            with temp_path.open("w") as fp:
                rapidjson.dump(
                    metadata, fp, default=self.np_encoder, number_mode=rapidjson.NM_NATIVE
                )
            temp_path.replace(self.global_metadata_path)
