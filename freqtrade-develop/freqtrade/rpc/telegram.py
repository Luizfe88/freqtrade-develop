...
class Telegram(RPCHandler):
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        """
        force_enter_text = (
            "*/forcelong <par> [<taxa>]:* `Compra instantaneamente o par fornecido. "
            "Opcionalmente aceita uma taxa/preço para compra "
            "(aplica-se apenas a ordens limit).` \n"
        )
        if self._rpc._freqtrade.trading_mode != TradingMode.SPOT:
            force_enter_text += (
                "*/forceshort <par> [<taxa>]:* `Vende instantaneamente (short) o par fornecido. "
                "Opcionalmente aceita uma taxa/preço para venda "
                "(aplica-se apenas a ordens limit).` \n"
            )
        message = (
            "_Controle do Bot_\n"
            ...
            "*/pause:* `Pausa novas entradas, mas gerencia as negociações abertas`\n"
            "*/stop:* `Para o trader`\n"
            "*/stopentry:* `Para de entrar em novas trocas, mas gerencia as abertas` \n"
            "*/forceexit <trade_id>|all:* `Sai instantaneamente de uma negociação ou de todas, "
            "independentemente do lucro`\n"
            "*/fx <trade_id>|all:* `Alias para /forceexit`\n"
            f"{force_enter_text if self._config.get('force_entry_enable', False) else ''}"
            "*/delete <trade_id>:* `Deleta instantaneamente a negociação no banco de dados`\n"
            "*/reload_trade <trade_id>:* `Recarrega a negociação a partir das ordens da exchange`\n"
            "*/cancel_open_order <trade_id>:* `Cancela ordens abertas para uma negociação.`\n"
            "*/coo <trade_id>|all:* `Alias para /cancel_open_order`\n"
            "*/whitelist [sorted] [baseonly]:* `Mostra a lista branca atual.`\n"
            "*/blacklist [par]:* `Mostra a lista negra atual ou adiciona novos pares.` \n"
            "*/blacklist_delete [pares]| /bl_delete [pares]:* "
            "`Remove par/padrão da lista negra. Reseta ao recarregar config.` \n"
            "*/reload_config:* `Recarrega o arquivo de configuração` \n"
            "*/unlock <par|id>:* `Desbloqueia este par (ou ID de bloqueio)`\n"
            "_Estado Atual_\n"
            "------------\n"
            "*/show_config:* `Mostra a configuração em execução` \n"
            "*/locks:* `Mostra pares bloqueados no momento`\n"
            "*/balance:* `Mostra o saldo gerenciado pelo bot por moeda`\n"
            "*/balance total:* `Mostra o saldo total da conta por moeda`\n"
            "*/logs [limite]:* `Mostra os logs mais recentes - padrão 10` \n"
            "*/count:* `Mostra o número de negociações ativas vs permitidas`\n"
            "*/health:* `Mostra o timestamp do último processo` \n"
            "*/marketdir [long | short | even | none]:* `Atualiza a direção do mercado.`\n"
            "*/list_custom_data <trade_id> <key>:* `Lista dados customizados para o ID e Chave.`\n"
            "_Estatísticas_\n"
            "------------\n"
            "*/status <trade_id>|[table]:* `Lista todas as negociações abertas`\n"
            "         *table :* `exibe as negociações em formato de tabela`\n"
            "*/entries <par|none>:* `Mostra o desempenho por tag de entrada`\n"
            "*/exits <par|none>:* `Mostra o desempenho por motivo de saída`\n"
            "*/mix_tags <par|none>:* `Mostra desempenho combinado (entrada + saída)`\n"
            "*/trades [limite]:* `Lista as últimas negociações fechadas (padrão 10)`\n"
            "*/profit [<n>]:* `Lucro acumulado dos últimos n dias`\n"
            "*/profit_long [<n>]:* `Lucro acumulado de posições LONG (n dias)`\n"
            "*/profit_short [<n>]:* `Lucro acumulado de posições SHORT (n dias)`\n"
            "*/performance:* `Desempenho por par (negociações finalizadas)`\n"
            "*/daily <n>:* `Lucro/prejuízo por dia nos últimos n dias`\n"
            "*/weekly <n>:* `Estatísticas semanais dos últimos n semanas`\n"
            "*/monthly <n>:* `Estatísticas mensais dos últimos n meses`\n"
            "*/stats:* `Estatísticas de vitórias/derrotas e durações médias`\n"
